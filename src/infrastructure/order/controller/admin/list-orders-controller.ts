...
import FindOrdersByStoreId, { OrderDetail } from '../../../../application/order/find-by-store-id/find-orders-by-store-id'
...
export default class ListOrdersController {
    constructor(private readonly findOrdersByStoreId: FindOrdersByStoreId, private readonly findStoreBySellerId: FindStoreBySellerId) { }
    ...
    handle = async (req: Request, res: Response): Promise<void> => {
        ...
        const ordersJsonApi = orders.map((order: OrderDetail) => {
            return JsonApiOrderDetailTransformer.transform(order)
        })
